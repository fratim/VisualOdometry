...
if (debug)
    ...
    imshow(img0)
    hold on
    scatter(S{2,1}(pplot,1),S{2,1}(pplot,2),'red')
    title('img0 matched features')

    subplot(2,2,2)
    imshow(img1)
    hold on
    scatter(S{1,1}(pplot,1),S{1,1}(pplot,2),'blue')
    ...
    hold on
    cam1 = plotCamera('Location',S{1,6}(1:3,4),'Orientation',S{1,6}(1:3,1:3),'Size',0.3);
    hold on
    cam2 = plotCamera('Location',S{2,6}(1:3,4),'Orientation',S{2,6}(1:3,1:3),'Size',0.3);
...
for i = range
    ...
    if length(keep(keep>0))<10
        disp('Less than 10 keypoints tracked, execution stopped')
        break
    end
